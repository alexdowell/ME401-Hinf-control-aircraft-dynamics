...
#    return ss(gtf)
    alpha = np.deg2rad(6)
    A = [[-.038, 18.984, 0, -32.17],[ -.001, -.632, 1, 0],[ 0, -.759, -.518, 0],[ 0, 0, 1, 0]]
    B = [[10.1, 0], [0, -.0086], [.025, -.011], [ 0, 0]]
    C = [[250, 0, 0, 0],[ 0, alpha, 0, 0]]
    D = [[0, 0], [0, 0]]
    return ss(A,B,C,D)
...
#    plt.plot(time, y2[1], label='des. 2 $y_2(t))$')
    plt.plot(time, y2[0], 'kd' , label='V(t)')
    plt.plot(time, y2[1], 'cd' , label='alpha(t)')
    plt.xlabel('time [s]')
    plt.ylabel('response ')
    plt.legend()
    plt.title('cloed loop step response')
