...
#    return ss(gtf)
    alpha = np.deg2rad(6)
    A = [[0, 0, 1, 0],[ .0487, -.0829, 0, -1],[ 0, -4.546, -1.699, .1717],[ 0, 3.382, -.0654, -.0893]]
    B = [[0, 0], [0, .0116], [27.276, .5758], [ .3952, -1.362]]
    C = [[0, 1, 0, 0],[ 0, 0, m.cos(alpha), m.sin(alpha)]]
    D = [[0, 0], [0, 0]]
    return ss(A,B,C,D)
...
def analysis():
    ...
    g = plant()
#    print('the poles are', control.pole(g))
...
#    wp3 = ss(weighting(wb=wb3, m=1.5, a=1e-4)) # extra
    wp = wp1.append(wp2)
    k, _, info = mixsyn(g, wp, wu)
#    k, _, info = mixsyn(g, wp1, wp2,wp3)
...
#    k1, gam1 = synth(0.25, 0.25) #original
...
#    _, gam3 = synth(25, 0.25) #original
    # equal weighting on each output
    k1, gam1 = synth(0.1, 0.1) 
    # increase "bandwidth" of output 2 by moving crossover weighting frequency 100 times higher
    k2, gam2 = synth(0.1, 1)
    # now weight output 1 more heavily
    # won't plot this one, just want gamma
...
#    # equal weighting on each output
#    k1, gam1 = synth(.25, .25, 1)
#    # increase "bandwidth" of output 2 by moving crossover weighting frequency 100 times higher
#    k2, gam2 = synth(0.25, 25, 1)
#    # now weight output 1 more heavily
#    # won't plot this one, just want gamma
#    _, gam3 = synth(25, 0.25, 1)    
...
#    print('design 3 gamma {:.3g} (Skogestad: 6.73)'.format(gam3))
    print('design 1 gamma {:.3g}'.format(gam1))
    print('design 2 gamma {:.3g}'.format(gam2))
    print('design 3 gamma {:.3g}'.format(gam3))
...
#    plt.semilogx(w, 20*np.log10(sv1[:, 1]), label=r'$\sigma_2(S_1)$')
    plt.semilogx(w, 20*np.log10(sv2[:, 0]), 'g1' , label=r'$\sigma_1(S)$')
    plt.semilogx(w, 20*np.log10(sv2[:, 1]), 'm1' , label=r'$\sigma_2(S)$')
    plt.ylim([-60, 10])
...
#    plt.plot(time, y2[1], label='des. 2 $y_2(t))$')
    plt.plot(time, y2[0],'kd' , label='p(t)')
    plt.plot(time, y2[1],'cd', label='beta(t)')
    plt.xlabel('time [s]')
    plt.ylabel('response')
    plt.legend()
